...
#define kPluginName "BalancePlus"
...
#define kPluginIdentifier "OpenFX.Yo.BalancePlus"
...
class BalancePlusProcessorBase
...
public:
    BalancePlusProcessorBase(ImageEffect &instance)
    ...
    virtual ~BalancePlusProcessorBase()
    {
    }
...
class ImageMinMaxMeanProcessor
    : public BalancePlusProcessorBase
{
private:
...
public:
    ImageMinMaxMeanProcessor(ImageEffect &instance)
        : BalancePlusProcessorBase(instance)
    {
...
class BalancePlusPlugin
    : public ImageEffect
{
public:
    /** @brief ctor */
    BalancePlusPlugin(OfxImageEffectHandle handle)
        : ImageEffect(handle)
        , _dstClip(0)
        , _srcClip(0)
        , _btmLeft(0)
...
private:
    ...
    void YoProcess(BalancePlusProcessorBase &processor, const Image* srcImg, double time, const OfxRectI &analysisWindow, 
...
void
BalancePlusPlugin::setupAndProcess(ImageScaler& p_ImageScaler, const RenderArguments &args)
{
...
void
BalancePlusPlugin::render(const RenderArguments &args)
{
...
} // BalancePlusPlugin::render

// override the roi call
// Required if the plugin requires a region from the inputs which is different from the rendered region of the output.
// (this is the case here)
void
BalancePlusPlugin::getRegionsOfInterest(const RegionsOfInterestArguments &args,
...
bool
BalancePlusPlugin::getRegionOfDefinition(const RegionOfDefinitionArguments &args,
                                             OfxRectD & /*rod*/)
{
    if ( !kSupportsRenderScale && ( (args.renderScale.x != 1.) || (args.renderScale.y != 1.) ) ) {
        throwSuiteStatusException(kOfxStatFailed);
...
bool
BalancePlusPlugin::isIdentity(const IsIdentityArguments &args,
                                  Clip * &identityClip,
                                  double & identityTime
...
void
BalancePlusPlugin::changedParam(const InstanceChangedArgs &args,
...
//} // BalancePlusPlugin::changedParam

/* set up and run a processor */
void
BalancePlusPlugin::YoProcess(BalancePlusProcessorBase &processor,
                                       const Image* srcImg,
                                       double time,
                                       const OfxRectI &analysisWindow,
                                       const Results &prevResults,
...
bool
BalancePlusPlugin::computeWindow(const Image* srcImg,
                                     double time,
                                     OfxRectI *analysisWindow)
{
    OfxRectD regionOfInterest;
...
// update image statistics
void
BalancePlusPlugin::update(const Image* srcImg,
                              double time,
                              const OfxRectI &analysisWindow)
{
...
void
BalancePlusPlugin::updateHSV(const Image* srcImg,
                                  double time,
                                  const OfxRectI &analysisWindow)
{
    Results results;
...
void
BalancePlusPlugin::updateLuma(const Image* srcImg,
                                  double time,
                                  const OfxRectI &analysisWindow)
{
    Results results;
...
BalancePlusPluginFactory::BalancePlusPluginFactory()
    : OFX::PluginFactoryHelper<BalancePlusPluginFactory>(kPluginIdentifier, kPluginVersionMajor, kPluginVersionMinor)
{
}


void
BalancePlusPluginFactory::describe(ImageEffectDescriptor &desc)
{
    // basic labels
...
void
BalancePlusPluginFactory::describeInContext(ImageEffectDescriptor &desc,
                                                ContextEnum /*context*/)
{
...
ImageEffect*
BalancePlusPluginFactory::createInstance(OfxImageEffectHandle handle, ContextEnum /*context*/)
{
    return new BalancePlusPlugin(handle);
}

void OFX::Plugin::getPluginIDs(PluginFactoryArray& p_FactoryArray)
{
    static BalancePlusPluginFactory balancePlusPlugin;
    p_FactoryArray.push_back(&balancePlusPlugin);
}
