...
"   	float w_BalR = p_BalR;             \n" \
"   	float w_BalB = p_BalB;             \n" \
"   	float w_LogBalR = p_LogBalR;       \n" \
"   	float w_LogBalB = p_LogBalB;       \n" \
"   	float w_WhiteA = p_WhiteA;         \n" \
"   	float w_WhiteB = p_WhiteB;			\n" \
"       SRC[0] = (w_WhiteA == 1.0f) && (w_WhiteB == 1.0f) ? SRC[0] + w_LogBalR : ((w_WhiteA == 1.0f) ? SRC[0] * w_BalR : SRC[0]);     \n" \
"       SRC[2] = (w_WhiteA == 1.0f) && (w_WhiteB == 1.0f) ? SRC[2] + w_LogBalB : ((w_WhiteA == 1.0f) ? SRC[2] * w_BalB : SRC[2]);     \n" \
"                                           \n" \
...
void RunOpenCLKernel(void* p_CmdQ, int p_Width, int p_Height, float* p_Bal, float* p_LogBal, 
...
//#define _DEBUG
    ...
    error |= clSetKernelArg(kernel, count++, sizeof(float), &p_Bal[0]);
    error |= clSetKernelArg(kernel, count++, sizeof(float), &p_Bal[1]);
    error |= clSetKernelArg(kernel, count++, sizeof(float), &p_LogBal[0]);
    error |= clSetKernelArg(kernel, count++, sizeof(float), &p_LogBal[1]);
	error |= clSetKernelArg(kernel, count++, sizeof(float), &p_White[0]);
	error |= clSetKernelArg(kernel, count++, sizeof(float), &p_White[1]);
