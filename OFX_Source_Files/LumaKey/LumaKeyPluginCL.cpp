...
#define kPluginName "LumaKey"
#define kPluginGrouping "OpenFX Yo"
#define kPluginDescription \
"------------------------------------------------------------------------------------------------------------------ \n" \
"LumaKeyer: Luma based keyer. Use eyedropper to isolate specific range and finetune with the \n" \
"controls."

#define kPluginIdentifier "OpenFX.Yo.LumaKey"
...
class LumaKey : public OFX::ImageProcessor
{
public:
    explicit LumaKey(OFX::ImageEffect& p_Instance);
...
LumaKey::LumaKey(OFX::ImageEffect& p_Instance)
    : OFX::ImageProcessor(p_Instance)
{
}
/*
extern void RunCudaKernel(int p_Width, int p_Height, float* p_LumaKey, float* p_Switch, const float* p_Input, float* p_Output);

void LumaKey::processImagesCUDA()
{
...
*/
extern void RunOpenCLKernel(void* p_CmdQ, int p_Width, int p_Height, float* p_LumaKey, float* p_Switch, const float* p_Input, float* p_Output);

void LumaKey::processImagesOpenCL()
{
    const OfxRectI& bounds = _srcImg->getBounds();
    const int width = bounds.x2 - bounds.x1;
    const int height = bounds.y2 - bounds.y1;
...
void LumaKey::multiThreadProcessImages(OfxRectI p_ProcWindow)
{
    for (int y = p_ProcWindow.y1; y < p_ProcWindow.y2; ++y)
    {
        ...
        for (int x = p_ProcWindow.x1; x < p_ProcWindow.x2; ++x)
        {
            ...
            if (srcPix)
            {
				...
				float k = a == 1.0f ? 0.0f : (a + b <= n ? 1.0f : (a <= n ? pow((n - a) / b, e) : 0));
				float alpha = k * w;
...
void LumaKey::setSrcImg(OFX::Image* p_SrcImg)
{
    _srcImg = p_SrcImg;
}

void LumaKey::setScales(float p_ScaleR, float p_ScaleG, float p_ScaleB, float p_ScaleA,
	float p_ScaleD, float p_ScaleE, float p_ScaleO, float p_ScaleZ, float p_SwitchA, float p_SwitchB)
{
    _scales[0] = p_ScaleR;
    _scales[1] = p_ScaleG;
...
{
public:
    explicit LumaKeyPlugin(OfxImageEffectHandle p_Handle);
    ...
    void setupAndProcess(LumaKey &p_LumaKey, const OFX::RenderArguments& p_Args);
...
LumaKeyPlugin::LumaKeyPlugin(OfxImageEffectHandle p_Handle)
...
void LumaKeyPlugin::render(const OFX::RenderArguments& p_Args)
{
    if ((m_DstClip->getPixelDepth() == OFX::eBitDepthFloat) && (m_DstClip->getPixelComponents() == OFX::ePixelComponentRGBA))
    {
        LumaKey LumaKey(*this);
        setupAndProcess(LumaKey, p_Args);
    }
    else
    {
...
bool LumaKeyPlugin::isIdentity(const OFX::IsIdentityArguments& p_Args, OFX::Clip*& p_IdentityClip, double& p_IdentityTime)
{
    ...
    if ((rScale == 1.0) && (gScale == 1.0) && (bScale == 0.0) && (aScale == 0.0) && 
		(dScale == 1.0) && (eScale == 1.0) && (oScale == 0.0) && (zScale == 0.0))
    {
...
void LumaKeyPlugin::changedParam(const OFX::InstanceChangedArgs& p_Args, const std::string& p_ParamName)
{
	...
	fprintf (pFile, "// LumaKeyPlugin DCTL export\n" \
	"\n" \
	"__DEVICE__ float3 transform(int p_Width, int p_Height, int p_X, int p_Y, float p_R, float p_G, float p_B)\n" \
	"{\n" \
	"	// switches for display LumaKey, invert alpha\n" \
	"	int display = %d;\n" \
	"	bool DisplayLumaKey = display == 1;\n" \
	"	int invert = %d;\n" \
	"	bool InvertAlpha = invert == 1;\n" \
	"\n" \
	"	// LumaKey parameter values\n" \
	"	float high = %ff;\n" \
	"	float highfade = %ff;\n" \
	"	float lowfade = %ff;\n" \
	"	float low = %ff;\n" \
	...
	"	float r = DisplayLumaKey ? alphaV : p_R;\n" \
	"	float g = DisplayLumaKey ? alphaV : p_G;\n" \
	"	float b = DisplayLumaKey ? alphaV : p_B;\n" \
	"\n" \
...
void LumaKeyPlugin::setupAndProcess(LumaKey& p_LumaKey, const OFX::RenderArguments& p_Args)
{
    // Get the dst image
    ...
    // Set the images
    p_LumaKey.setDstImg(dst.get());
    p_LumaKey.setSrcImg(src.get());

    // Setup OpenCL and CUDA Render arguments
    p_LumaKey.setGPURenderArgs(p_Args);

    // Set the render window
    p_LumaKey.setRenderWindow(p_Args.renderWindow);

    // Set the scales
    p_LumaKey.setScales(rScale, gScale, bScale, aScale, dScale, eScale, oScale, zScale, aSwitchF, bSwitchF);
...
LumaKeyPluginFactory::LumaKeyPluginFactory()
    : OFX::PluginFactoryHelper<LumaKeyPluginFactory>(kPluginIdentifier, kPluginVersionMajor, kPluginVersionMinor)
{
}

void LumaKeyPluginFactory::describe(OFX::ImageEffectDescriptor& p_Desc)
{
    // Basic labels
...
void LumaKeyPluginFactory::describeInContext(OFX::ImageEffectDescriptor& p_Desc, OFX::ContextEnum /*p_Context*/)
{
    ...
    componentScalesGroup->setLabels("Luma Keyer", "Luma Keyer", "Luma Keyer");
    ...
    boolParam->setHint("Displays LumaKey on RGB Channels");
    boolParam->setLabels("Display LumaKey", "Display LumaKey", "Display LumaKey");
...
ImageEffect* LumaKeyPluginFactory::createInstance(OfxImageEffectHandle p_Handle, ContextEnum /*p_Context*/)
{
    return new LumaKeyPlugin(p_Handle);
}

void OFX::Plugin::getPluginIDs(PluginFactoryArray& p_FactoryArray)
{
    static LumaKeyPluginFactory LumaKeyPlugin;
    p_FactoryArray.push_back(&LumaKeyPlugin);
}
